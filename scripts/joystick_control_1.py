...
# ロボットへの速度指令計算
	...
	if joy_msg.buttons[3]:
	    ...
	    if robot_goal_theta > math.pi/6 and min(tmp[50:120]) > 0.22:
		cmdvel.angular.z = 0.6*math.pi*(robot_goal_theta)
		cmdvel.linear.x = 0.1
	    elif robot_goal_theta < -math.pi/6 and min(tmp[-120:-50]) > 0.22:
		cmdvel.angular.z = 0.6*math.pi*(robot_goal_theta)
		cmdvel.linear.x = 0.1
	    else:
		cmdvel.angular.z = 0.1*math.pi*(robot_goal_theta)/abs(robot_goal_theta)
		cmdvel.linear.x = 0.2
	    if abs(pos_goal.point.y - robot_y) < 0.2 and abs(pos_goal.point.x - robot_x) < 0.2:
		cmdvel.linear.x = 0
		cmdvel.angular.z = 0
